...
import { ClipboardCheck, CheckCircle, XCircle, Zap, BookOpen, ShieldCheck } from "lucide-react";
...
export default function QAReviewPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="solid-card p-5 border-l-4 border-l-[var(--accent-primary)]">
        <div className="flex items-start gap-3">
          <div className="w-10 h-10 rounded-[var(--radius-md)] bg-[var(--accent-primary)]/10 flex items-center justify-center shrink-0">
            <ShieldCheck className="w-5 h-5 text-[var(--accent-primary)]" />
          </div>
          <div>
            <h1 className="text-2xl font-bold font-display">Reviewer Desk</h1>
            <p className="text-sm text-[var(--text-muted)] mt-1">
              Validate bank complaint responses before they are approved for customer communication and model feedback.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="solid-card p-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">Queue</p>
          <p className="text-2xl font-bold font-display text-[var(--accent-primary)]">{queue.length}</p>
        </div>
        <div className="solid-card p-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">Checked</p>
      ...
      {pending.length === 0 ? (
        <motion.div
          ...
          className="solid-card p-12 text-center"
        >
          ...
          <p className="text-lg font-semibold font-display">All Cases Reviewed</p>
          <p className="text-sm text-[var(--text-muted)]">No pending worker responses require QA approval.</p>
        </motion.div>
      ) : (
            ...
            <motion.div key={item.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="p-4 rounded-[var(--radius-md)] bg-[var(--bg-inset)] border border-[var(--border-subtle)] mb-4">
                <p className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider mb-2 flex items-center gap-1">
                  <Zap className="w-3 h-3" /> Proposed Response
                </p>
                ...
                {item.rag_sources?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-[var(--border-subtle)]">
                    ...
                    {item.rag_sources.map((s, j) => (
                      <span key={j} className="px-2 py-0.5 rounded-[var(--radius-sm)] text-[9px] bg-[var(--accent-primary)]/8 text-[var(--accent-primary)]">
                        {typeof s === "string" ? s : String(s["title"] || s["source"] || s["id"] || `Source ${j + 1}`)}
                      </span>
                    ))}
                  </div>
                )}
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => handleDecision(item, "approve_for_rlhf")}
                  ...
                  <CheckCircle className="w-4 h-4" /> Approve Response
                </button>
